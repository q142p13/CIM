...
for step=1:steps
    ...
    for sub=1:len
        ...
        D(j,:) = D(j,:) - yita * a5 * D(j,:) - yita * a1 * (S*D(j,:)'-X(:,j))' * S - yita * (b.data - A(i,j,k,t)) * ttm(ttm(ttm(G,S(i,:),1),C(k,:),3),T(t,:),4);
        D(j,:) = D(j,:) - yita * a5 * D(j,:) - yita * (b.data - A(i,j,k,t)) * ttm(ttm(ttm(G,S(i,:),1),C(k,:),3),T(t,:),4);
        C(k,:) = C(k,:) - yita * a5 * C(k,:)  - yita * (b.data - A(i,j,k,t)) * permute(ttm(ttm(ttm(G,S(i,:),1),D(j,:),2),T(t,:),4),[1 3 2 4]);
        T(t,:) = T(t,:) - yita * a5 * T(t,:) - yita * (b.data - A(i,j,k,t)) * permute(ttm(ttm(ttm(G,S(i,:),1),D(j,:),2),C(k,:),3),[1 4 3 2]);
